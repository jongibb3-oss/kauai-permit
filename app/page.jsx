...
function Input({value,onChange,placeholder,type="text"}){return <input type={type} value={value} onChange={onChange} placeholder={placeholder} style={inputStyle} onFocus={e=>e.target.style.borderColor="#4a7c59"} onBlur={e=>e.target.style.borderColor="#c8dfc7"}/>;}
...
- In SMA: ${form.inSMA}, Flood Zone: ${form.inFloodZone}, Historic: ${form.historicProperty}, Near Waterway: ${form.nearStream}`;

    try{
      const response=await fetch("/api/review",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages:[{role:"user",content:prompt}]})});
  ...
  return(
    <div>
      ...
      {status==="done"&&(
        <div>
          {result&&<div style={{background:"#fff",border:"1.5px solid #b5d9b8",borderRadius:12,padding:"1.5rem"}}>
          ...
          {rawError&&<div style={{background:"#fff5f5",border:"1.5px solid #f5c6c6",borderRadius:8,padding:"1.2rem",color:"#8b2020",fontFamily:"monospace",fontSize:"0.8rem"}}><strong>API Error:</strong> {rawError}</div>}
